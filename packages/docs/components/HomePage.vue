...
onMounted(() => {
  document.addEventListener('scroll', function (e) {
    ...
    if (sdk_nav) {
      sdk_nav.classList.add('scroll-sdk-nav')
      // 如果回到顶部 删除背景颜色
      if (window.scrollY === 0) {
        sdk_nav.classList.remove('scroll-sdk-nav')
      }
    }
  })
})
...
const supportFrameworks = [
  ...
  {
    src: '/images/laya.webp',
  },
  {
    src: '/images/cocos.webp',
  },
  {
...
<template>
  ...
  <section class="p-20"
    ...
    <div class="flex flex-col items-center justify-center text-white text-5xl font-bold gap-10 m-auto"
      ...
      <div class="grid md:grid-cols-4 justify-between gap-10">
        <img v-for="item in supportFrameworks" :key="item.src" class="" :src="item.src" alt="">
      </div>
